...
<html lang="en">
    ...
    <body>
        ...
        <jsp:include page="/admin/sidebar.jsp">
            <jsp:param name="currentPage" value="revenue-chart" />
        </jsp:include>
        ...
        <script>
            ...
            const monthList = [];
            <% 
    List<RevenueByMonth> mList = (List<RevenueByMonth>) request.getAttribute("revenueList");
    if (mList != null) {
        for (RevenueByMonth r : mList) {
            %>
            monthList.push({month: <%= r.getMonth() %>, revenue: <%= r.getRevenue() %>});
            <% 
        }
    }
            %>

            console.log("monthList data:", monthList);

            if (monthList.length > 0) {

                const monthLabels = monthList.map(item => 'Tháng ' + item.month);
                const monthData = monthList.map(item => item.revenue);
                const monthColors = monthList.map(() => 'rgba(75, 192, 192, 0.6)');
            ...
            if (Object.keys(orderStatusSummary).length > 0) {
                ...
                statusLabels.forEach(label => {
                    if (label.includes('Chờ duyệt')) {
                        statusColors.push('rgba(54, 162, 235, 0.7)'); // Xanh dương
                    } else if (label.includes('đóng gói')) {
                        statusColors.push('rgba(255, 206, 86, 0.7)'); // Vàng
                    } else if (label.includes('vận chuyển')) {
                        statusColors.push('rgba(75, 192, 192, 0.7)'); // Xanh ngọc
                    } else if (label.includes('giao hàng')) {
                        statusColors.push('rgba(153, 102, 255, 0.7)'); // Tím
                    } else if (label.includes('thanh toán')) {
                        statusColors.push('rgba(40, 167, 69, 0.7)'); // Xanh lá
                    } else if (label.includes('hủy')) {
                        statusColors.push('rgba(255, 99, 132, 0.7)'); // Đỏ
                    } else if (label.includes('thiết kế riêng') && label.includes('duyệt')) {
                        statusColors.push('rgba(255, 159, 64, 0.7)'); // Cam
                    } else if (label.includes('thiết kế riêng') && label.includes('từ chối')) {
                        statusColors.push('rgba(201, 203, 207, 0.7)'); // Xám
                    } else {
                        statusColors.push('rgba(100, 100, 100, 0.7)'); // Màu fallback cho trạng thái chưa định nghĩa
                    }
                });
